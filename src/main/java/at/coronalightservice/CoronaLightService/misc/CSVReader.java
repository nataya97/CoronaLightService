...
public class CSVReader {
    ...
    private void getStates() throws IOException {
        ...
        while ((entries = reader.readNext()) != null) {
            ...
            System.out.println(Arrays.toString(splitEntries) + " " + splitEntries[2]);
            State state = new State(Long.parseLong(splitEntries[7]), splitEntries[8]);
            System.out.println("The State is... " + state.getState() + " " + state.getStateId());
            if(stateController.getStateByName(state.getState()).isEmpty()) {
                stateController.addState(state);
            }
            covidCases_stateController.addCovidCase(new CovidCases_State(stateController.getStateById(state.getStateId()), LocalDate.parse(splitEntries[0], formatter), Long.parseLong(splitEntries[1]),
    ...
    private void readCovidFaelle_District() throws IOException {
        ...
        FileOutputStream fileOutputStream = new FileOutputStream("CovidFaelle_GKZ.csv"); //TODO: ohne output
    ...
    private void getDistricts() throws IOException {
        ...
        while ((entries = reader.readNext()) != null) {
            splitEntries = entries[i].split(";");
            System.out.println(Arrays.toString(splitEntries) + " " + splitEntries[2]);
            State state = stateController.getStateById(Long.parseLong(String.valueOf(splitEntries[1].charAt(0))));
            District district = new District(splitEntries[0], Long.parseLong(splitEntries[1]), state);
            if(districtController.getDistrictByState(district.getDistrict()).isEmpty()) {
                districtController.addDistrict(district);
            }
            covidCases_districtController.addCovidCase(new CovidCases_District(districtController.getDistrictByGkz(Long.parseLong(splitEntries[1])), Long.parseLong(splitEntries[2]), Long.parseLong(splitEntries[3]),
                    Long.parseLong(splitEntries[4]), Long.parseLong(splitEntries[5])));
        }
    }

    @Bean
    private void readCovidFaelle_Timeline_District() throws IOException {
